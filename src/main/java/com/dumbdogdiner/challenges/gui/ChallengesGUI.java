package com.dumbdogdiner.challenges.gui;
...
public class ChallengesGUI {
	...
	public void openChallengesGUI(Player player) {
		...
		Inventory challengesGUI = Bukkit.createInventory(null, config.getInt("gui.size"),
				Util.color(config.getString("gui.name")));
		...
		for (Challenge challenge : challengesInGUI) {
			...
			for (String string : config.getStringList(iconPath + ".lore")) {
				string = string.replace("%date%", Util.getDateMessage())
						.replace("%time%", Util.timeMessage(ChallengeTimeUpdater.counter))
						.replace("%player%", player.getName())
						.replace("%counter%", Integer.toString(challenge.getCounters().get(player.getName())))
						.replace("%rank%", Integer.toString(challenge.getRanking(player.getName())));
				int i = 1;
				for (String playerName : challenge.getCounters().keySet()) {
					string = string.replace("%player" + Integer.toString(i) + "%", playerName).replace(
							"%counter" + Integer.toString(i) + "%",
							Integer.toString(challenge.getCounters().get(playerName)));
					if (Util.hasFactions()) {
						FPlayer fplayer = FPlayers.getInstance()
								.getByOfflinePlayer(Bukkit.getOfflinePlayer(playerName));
	...
	public ItemStack createIcon(String iconPath, List<String> list) {
		FileConfiguration config = Core.instance.getConfig();
		ItemStack icon = Util.createItemStack(Material.valueOf(config.getString(iconPath + ".material")),
				config.getInt(iconPath + ".amount"), config.getString(iconPath + ".name"),
				config.getInt(iconPath + ".data"), list);
		return icon;
	}
}
